...
folder are mapped to the application's root namespace, 'babymonkey'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "babymonkey"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'babymonkey' in the same way
as `"babymonkey/sass/src"`.
